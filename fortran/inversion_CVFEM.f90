program nsf_rift
	...
	integer :: first, last
	integer (kind = 4), parameter :: g_num = 42
	real ( kind = 8 ), dimension (2,42) :: g_xy
	
	real ( kind = 8 ), dimension (42,1) :: xcc, ycc

	integer, dimension (4000,3) :: t_node
	! integer, dimension (4000,3) :: t_new
	integer, dimension (40,3) :: c

	integer ( kind = 4 ) tri_num
	integer ( kind = 4 ) ierror
	integer ( kind = 4 ) ind(g_num)
	integer ( kind = 4 ) nod_tri(3,40)
	integer ( kind = 4 ) tnbr(3,40)
	integer ( kind = 4 ) tcc(3,40)
	...
	!uisng geompack to generate delaunay triangulation
	do i = 1, g_num
		g_xy(:,i) =  [x(i), y(i)]
	end do

	! print *, shape(g_xy)
	! g_xy = reshape((/&x, &y/), (/2, g_num/))
	do i = 1, g_num
		ind(i) = i
	end do
	...
	do jj = 1, cols-1
		...
		! print *, last - first
		c = transpose(tcc)
		! print *, shape(t_node(1:40,:))
		! t_node(:,((jj-1)*40+1):40*jj) = tcc
		t_node(((jj-1)*40+1):40*jj,:) = c
		first=first+ncol(jj)
	...
	! 	print *, xcc(i,:)
	! end do
	! t_new = reshape(t_node, (/4000,3/))
	! print *, shape(t_node)

	OPEN(UNIT=1, FILE="output.txt", ACTION="write", STATUS="replace")
  	do i=1,4000
    	write(1,*) (t_node(i,j), j=1,3)
  	end do
  	close(1)
